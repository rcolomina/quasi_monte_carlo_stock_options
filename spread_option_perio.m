...
function f_retorno = spread_option_perio(glp,w1,w2,rate,K,S10,S20,delta1,delta2,sigma1,sigma2,ro12,T,param)
	  value=0;
     N=length(glp);
	  for i=1:N
				u1=f_periodifica(glp(i,1));
            u2=f_periodifica(glp(i,2));
            value_aux=hstar(u1,u2,w1,w2,rate,delta1,delta2,S10,S20,sigma1,sigma2,ro12,T,K);
            value_aux=value_aux*f_periodifica_deriv(glp(i,1))*f_periodifica_deriv(glp(i,2));
